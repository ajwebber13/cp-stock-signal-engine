...
from data_pipeline import fetch_ticker, add_indicators, FEATURE_COLS, WATCHLIST
...
def load_ensemble():
    ...
    if not os.path.exists(path):
        raise FileNotFoundError("No trained ensemble found. Run model_trainer.py first.")
    return joblib.load(path)


# ── Signal Scoring ───────────────────────────────────────────────────────────

def score_ticker(symbol: str, ensemble) -> dict | None:
    ...
    return {
        ...
        "above_200d":   latest["Close"] > latest["sma_200"],
...
def format_summary(signals: list) -> str:
    """Daily digest when no single ticker clears threshold."""
...
def run_daily_scan(watchlist: list = WATCHLIST):
    print(f"\n{'='*50}")
    ...
    for sym in watchlist:
        ...
        if signal:
            ...
            label = f"{signal['prob']*100:.1f}% {'← SIGNAL' if signal['signal'] else ''}"
            print(label)
        else:
    ...
    # Fire individual alerts for tickers above threshold
    fired = 0
    ...
    # If nothing fired threshold, send watchlist summary
    if fired == 0:
...
def print_feature_importance():
    """Show which features XGBoost and LightGBM rely on most."""
    try:
        xgb_model = joblib.load(f"{MODEL_DIR}/xgboost.pkl")
        lgb_model = joblib.load(f"{MODEL_DIR}/lightgbm.pkl")
        ...
        print("\n📊 Top Feature Importances (XGBoost + LightGBM avg):")
        print("-" * 40)
        for feat, score in combined.head(10).items():
            bar = "█" * int(score * 100 / combined.max() * 20)
